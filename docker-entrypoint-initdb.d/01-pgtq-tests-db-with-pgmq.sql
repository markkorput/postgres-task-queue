...
DECLARE
    sql TEXT;
    dir TEXT := UPPER(direction);
    qtable TEXT := pgmq.format_table_name(queue_name, 'q');
    minmax TEXT;
    comp TEXT;
BEGIN
    IF dir NOT IN ('ASC', 'DESC') THEN
      RAISE EXCEPTION 'Invalid ordering direction: % (must be ASC or DESC)', direction;
    END IF;

    -- Set aggregation based on direction
    IF dir = 'ASC' THEN
        minmax := 'MIN';
        comp := '>=';
    ELSE
    ...
    sql := FORMAT(
        $QUERY$
        WITH fifo_groups AS (
            -- Determine the head message id per FIFO group based on direction
            SELECT
                COALESCE(headers->>'x-pgmq-group', '_default_fifo_group') AS fifo_key,
                %1$s(msg_id) AS head_msg_id
            FROM pgmq.%2$I
        ...
        eligible_groups AS (
            ...
            SELECT
                g.fifo_key,
                g.head_msg_id,
                ROW_NUMBER() OVER (ORDER BY g.head_msg_id %3$s) AS group_priority
            FROM fifo_groups g
            JOIN pgmq.%4$I h ON h.msg_id = g.head_msg_id
        ...
        available_messages AS (
            ...
            WHERE m.vt <= clock_timestamp()
              AND m.msg_id %7$s eg.head_msg_id
        ),
        ordered_messages AS (
        ...
        selected_messages AS (
            ...
            JOIN pgmq.%8$I m ON m.msg_id = om.msg_id
        ...
        $QUERY$,
        minmax, qtable, dir, qtable, dir, qtable, comp, qtable, qtable, make_interval(secs => vt)
    );
...
BEGIN
    ...
    LOOP
      ...
      FOR r IN
        SELECT * FROM pgmq.read_grouped_rr(queue_name, vt, qty, direction)
      LOOP
        RETURN NEXT r;
      END LOOP;
      IF FOUND THEN
...
---- PGTQ PATCH: added direction argument
...
DECLARE
    ...
    qtable TEXT := pgmq.format_table_name(queue_name, 'q');
BEGIN
    IF UPPER(direction) NOT IN ('ASC', 'DESC') THEN
      RAISE EXCEPTION 'Invalid ordering direction: % (must be ASC or DESC)', direction;
    END IF;
    ...
    LOOP
      ...
      sql := FORMAT(
          ...
          SELECT * FROM updated ORDER BY msg_id %s
          $QUERY$,
          qtable, conditional, direction, qtable, make_interval(secs => vt), direction
      );
